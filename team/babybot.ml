...
let bot c =
  while true do
    ...
    let rider_tiles = List.map (fun x-> x.tile) (List.append opp_riders rider_list) in
    ...
    let (sh_opp_team,inv_opp_team) = List.partition (fun (i,n) -> i=Shield) opp_items in
    let (sh_opp_num,inv_opp_num) = (snd (List.hd sh_opp_team),snd (List.hd inv_opp_team)) in
	
    let field_data = match get_status (FieldItemsStatus) with FieldItemsData a -> a
    ...
    List.iter (fun x -> 
      ...
      then ( let _ = send_action (UseItem(x.id, Shield)) in sh_num := !sh_num - 1 )
      else ()) rider_list;

    (*check North South West East and see if it is not blocked by tail or rider or out of bound*)
    let check_around rider ori =
      ...
      let b_tile = List.filter (fun (c,r) -> c >=0 && r >=0 && c <cNUM_COLUMNS && r <cNUM_ROWS) around in
      let bound_tile = (*check boundary conditions*)
	List.map (fun x -> if x=(c-1,r) then West  else if x=(c+1,r) then East 
	  else if x=(c,r+1) then South else North) b_tile 
      in

      let o_tile = List.filter (fun x -> not(List.mem x tail_data) && not(List.mem x rider_tiles)) b_tile in
      let open_tile = (*check if tiles are empty*)
	List.map (fun x -> if x=(c-1,r) then West  else if x=(c+1,r) then East 
	  else if x=(c,r+1) then South else North) o_tile 
      in

      let next_tile = if ori = North then (c,r-1) else if ori = South then (c,r+1) else if ori = West then (c-1,r) else (c+1,r) in
      let next_open_tile = (*open tiles of next move*)
	let b_tile =  List.filter (fun (c,r) -> c >=0 && r >=0 && c <cNUM_COLUMNS && r <cNUM_ROWS) (diamond next_tile) in
	List.filter (fun x -> not(List.mem x tail_data) && not(List.mem x rider_tiles)) b_tile
      in
      let orient =
	if List.length next_open_tile < 1 then (*dead end*)
	  (let choice = List.filter (fun x -> x <> ori) open_tile in
	   if choice = [] then ori else List.nth choice (get_random_num (List.length choice)))
	else if List.length open_tile = 0 then (*no open tile exists*)
	...
	if coor = (c,r-2) || coor = (c -1, r -1) then (if dir then North else West)
	else if coor = (c +1, r -1) || coor = (c+2, r) then (if dir then East else South)
	else if coor = (c-2, r) || coor = (c -1, r +1) then (if dir then West else North)
	else (if dir then South else East)
      in

      let pos = [(c,r-2);(c -1, r -1);(c +1, r -1);(c-2, r);(c+2, r);(c, r+2);(c -1, r +1);(c +1, r +1)] in
      let diamond = List.filter (fun (col,row) -> col >= 0 && row >= 0 && col < cNUM_COLUMNS && row < cNUM_ROWS) pos in
      let enemies = List.filter (fun e -> List.mem e.tile diamond) opp_riders in
      let team_riders = List.filter (fun r -> List.mem r.tile diamond) rider_list in

      if List.length enemies > 0 then
	(let inv_enemies = List.filter (fun x -> (List.mem Invincible x.modifiers)) enemies in
	 let shi_enemies = List.filter (fun x -> List.mem Shielded x.modifiers) enemies in
	 let en_score = 
	 ...
	 if my_score > en_score then (* attack enemy for 1 step *)
	   ((if (my_score = 3 && not(List.mem Invincible rider.modifiers) && !inv_num > 0)
	     then (
	       let _ = send_action (UseItem(id, Invincibility)) in inv_num := !inv_num - 1)
	     else ());
	...
	compare c d) field_data) in
      
      (if (List.length field_min) > 0 
      then
	let field_star = (a_star rider.tile (let (_,z)=(List.hd field_min) in z) tail_data manhattan) in
	(moveTo rider rider.tile (if List.length field_star > 0 then (List.nth field_star 1) else rider.tile))
      else rider.orientation)
	(*item*)
      )
    in

    (*check the positions around and react*)
    List.iter 
      (fun x -> 
